...
void Dense::backward_pass(float learning_rate)
{
    ...
    CUBLAS_CALL( cublasSaxpy(cublasHandle, in_size * out_size, &eta,
                             grad_weights, 1, weights, 1) );

    // TODO (set 5): biases = biases + eta * grad_biases
    CUBLAS_CALL( cublasSaxpy(cublasHandle, out_size, &eta, grad_biases, 1,
                             biases, 1) );
}
...
void Conv2D::forward_pass()
{
    ...
    CUDNN_CALL( cudnnConvolutionForward(cudnnHandle, &one, in_shape, in_batch,
                                        filter_desc, weights, conv_desc,
...
void Conv2D::backward_pass(float learning_rate)
{
    ...
    CUDNN_CALL( cudnnConvolutionBackwardFilter(cudnnHandle, &one, in_shape,
                                               in_batch, out_shape,
                                               grad_out_batch, conv_desc,
                                               bwd_filter_algo, workspace,
                                               workspace_size, &zero,
    ...
    CUDNN_CALL( cudnnConvolutionBackwardData(cudnnHandle, &one, filter_desc,
                                             weights, out_shape, grad_out_batch,
                                             conv_desc, bwd_data_algo,
                                             workspace, workspace_size, &zero,
    ...
    CUBLAS_CALL( cublasSaxpy(cublasHandle, n_weights, &eta, grad_weights, 1,
                             weights, 1) );

    // TODO (set 6): biases = biases + eta * grad_biases
    CUBLAS_CALL( cublasSaxpy(cublasHandle, n_biases, &eta, grad_biases, 1,
                             biases, 1) );
}
...
: Layer(prev, cublasHandle, cudnnHandle)
{
    ...
    CUDNN_CALL( cudnnSetPooling2dDescriptor(pooling_desc, mode,
                                            CUDNN_PROPAGATE_NAN, stride,
                                            stride, 0, 0, stride, stride) );
...
void Pool2D::forward_pass()
{
    ...
    CUDNN_CALL( cudnnPoolingForward(cudnnHandle, pooling_desc, &one, in_shape,
...
void Pool2D::backward_pass(float learning_rate)
{
    ...
    CUDNN_CALL( cudnnPoolingBackward(cudnnHandle, pooling_desc, &one, out_shape,
                                     out_batch, out_shape, grad_out_batch,
                                     in_shape, in_batch, &zero, in_shape,
                                     grad_in_batch) );
}
...
void SoftmaxCrossEntropy::backward_pass(float lr)
{
    ...
    CUBLAS_CALL( cublasScopy(cublasHandle, size, out_batch, 1,
                             grad_in_batch, 1) );

    // TODO (set 5): set grad_in_batch = grad_in_batch - grad_out_batch using
    //               cublasSaxpy
    CUBLAS_CALL( cublasSaxpy(cublasHandle, size, &minus_one, grad_out_batch,
                             1, grad_in_batch, 1) );
