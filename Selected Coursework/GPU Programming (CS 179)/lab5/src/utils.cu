    ...
    int n, int c, int h, int w)
{
    ...
    // have each thread in each block accumulate some of the total cross-entropy
    // in shared memory
    shmem[tid] = 0.0;
    for (; idx < n; idx += blockDim.x * gridDim.x)
    {
        ...
        // Calculate -log(pred_Y[i]) * true_Y[i] values and add them to shared
        // memory at tid.
        float minus_log_pred = 0;
        float pred_true_product = 0;
        for (unsigned j = 0; j < c * h * w; ++j)
        {
            minus_log_pred = -log(pred_Y[idx_cur + j]);
            pred_true_product = minus_log_pred * true_Y[idx_cur + j];

            // Add the result to shared memory.
            shmem[tid] += pred_true_product;
        }
    }
    __syncthreads();

    // do a reduction to sum up all of the cross-entropy components in this
