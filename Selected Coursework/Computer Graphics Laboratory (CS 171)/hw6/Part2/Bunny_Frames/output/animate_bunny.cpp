...
void init(int mode) {
    ...
    objects = read_objects(scene_filename);
    original_objects = read_objects(scene_filename);
...
void display(void) {
    ...
    glTranslatef(-cam.pos[0], (int) -cam.pos[1]/2 , -cam.pos[2]);
...
void key_pressed(unsigned char key, int x, int y) {
    ...
    /* If the 'l' key is pressed, change the scene to display the next frame
     * object in the animation. The animation loops back to the 0th object if
     * 'l' key is pressed while the last frame of the animation is being
     * displayed.
     */
    else if (key == 'l') {
        scene_number++;
        ...
        if (scene_number < 10) {
            scene_filename = (string) "scene_bunny0" + to_string(scene_number)
                             + ".txt";
        }
        else {
            scene_filename = (string) "scene_bunny" + to_string(scene_number)
                             + ".txt";
        }
        init(mode);
        glutPostRedisplay();
        ...
        if (scene_number < 10) {
            cout << "bunny0" << scene_number << " displayed" << endl;
        }
        else {
            cout << "bunny" << scene_number << " displayed" << endl;
        }
    }
    else {
...
int main(int argc, char* argv[])
{
    ...
    scene_filename = "scene_bunny00.txt";
    scene_number = 0;
