...
void display(void)
{
    ...
    for(int i = 0; i < vertices.size(); i++)
    {
        ...
        /* Update the current vertex's position and velocity using the discrete
         * Lagrangian of the system.
         */
        float updated_x =
            vertices[i].x + tstep * vertices[i].vx
            + (tstep * tstep / vertices[i].mass) * vertices[i].fx;
        ...
        float updated_y =
            ...
            + (tstep * tstep / vertices[i].mass) * vertices[i].fy;

        float updated_vy =
            vertices[i].vy + (tstep / vertices[i].mass) * vertices[i].fy;

        vertices[i].x = updated_x;
        vertices[i].y = updated_y;
        vertices[i].vx = updated_vx;
        vertices[i].vy = updated_vy;
