...
// November 30, 2018
// single_spring_pendulum_demo.cpp
...
/**
 * Update the pendulum's position and momentum using the discrete Lagrangian of
 * the system.
 */
void update_pendulum()
{
    float updated_x =
        m1.x + (dt * m1.px / m1.m) - (dt * dt * m1.k * m1.x / m1.m)
        + (dt * dt * m1.k * m1.rl * m1.x
           / (m1.m * sqrt(m1.x * m1.x + m1.y * m1.y)));

    float updated_px =
        m1.px - (dt * m1.k * m1.x)
        + (dt * m1.k * m1.rl * m1.x / sqrt(m1.x * m1.x + m1.y * m1.y));

    float updated_y =
        m1.y + (dt * dt * g) + (dt * m1.py / m1.m)
           - (dt * dt * m1.k * m1.y / m1.m)
           + (dt * dt * m1.k * m1.rl * m1.y
              / (m1.m * sqrt(m1.x * m1.x + m1.y * m1.y)));

    float updated_py =
        m1.py + dt * (g * m1.m + m1.k * m1.y
                      * ((1.0 / sqrt(m1.x * m1.x + m1.y * m1.y)) - 1.0));
