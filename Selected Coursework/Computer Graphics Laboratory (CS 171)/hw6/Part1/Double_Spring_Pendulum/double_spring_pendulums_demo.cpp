...
// November 30, 2018
// double_spring_pendulum_demo.cpp
...
/**
 * Update the double pendulum weights' positions and momentum values using the
 * discrete Lagrangian of the system.
 */
void update_pendulums()
{
    float spring_length_1 = sqrt(m1.x * m1.x + m1.y * m1.y);
    float spring_length_2 = sqrt((m1.x - m2.x) * (m1.x - m2.x)
                             + (m1.y - m2.y) * (m1.y - m2.y));

    float updated_x1 =
        m1.x + (dt / m1.m) * (m1.px - (dt * m1.k * m1.x
        * (spring_length_1 - m1.rl) / spring_length_1) - (dt * m2.k
        * (m1.x - m2.x) * (spring_length_2 - m2.rl) / spring_length_2));

    float updated_y1 =
        m1.y + (dt / m1.m) * (dt * g * m1.m + m1.py - (dt * m1.k * m1.y
        * (spring_length_1 - m1.rl) / spring_length_1) - (dt * m2.k
        * (m1.y - m2.y) * (spring_length_2 - m2.rl) / spring_length_2));

    float updated_x2 =
        m2.x + (dt / m2.m) * (m2.px + (dt * m2.k * (m1.x - m2.x)
        * (spring_length_2 - m2.rl) / spring_length_2));

    float updated_y2 =
        m2.y + (dt / m2.m) * (dt * g * m2.m + m2.py + (dt * m2.k * (m1.y - m2.y)
        * (spring_length_2 - m2.rl) / spring_length_2));

    float updated_px1 =
        m1.px - ((-dt * m2.k * m2.rl * m1.x * spring_length_1
        + dt * m2.k * m2.rl * m2.x * spring_length_1
        - dt * m1.k * m1.rl * m1.x * spring_length_2
        + dt * m1.k * m1.x * spring_length_1 * spring_length_2
        + dt * m2.k * m1.x * spring_length_1 * spring_length_2
        - dt * m2.k * m2.x * spring_length_1 * spring_length_2)
        / (spring_length_1 * spring_length_2));

    float updated_py1 =
        m1.py + dt * g * m1.m - (dt * m1.k * m1.y * (spring_length_1 - m1.rl)
        / spring_length_1) - (dt * m2.k * (m1.y - m2.y)
        * (spring_length_2 - m2.rl) / spring_length_2);

    float updated_px2 =
        m2.px + (dt * m2.k * (m1.x - m2.x) * (spring_length_2 - m2.rl)
        / spring_length_2);

    float updated_py2 =
        m2.py + dt * g * m2.m + (dt * m2.k * (m1.y - m2.y)
        * (spring_length_2 - m2.rl) / spring_length_2);
