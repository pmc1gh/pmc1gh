    ...
    def unsupervised_learning(self, X, N_iters):
        ...
        for k in range(1): # N_iters
            for j in range(len(X)):
                alphas_list.append(self.forward(X[j], normalize=True))
                betas_list.append(self.backward(X[j], normalize=True))
                    ...
                    for i in range(len(X)):
                        for j in range(1, len(X[i]) + 1):
                            ...
                            for ap in range(self.L):
                                total1 += alphas_list[i][j][ap] \
                                          * betas_list[i][j][ap]
                            # nO += alphas_list[i][j] * O[j+1][j+1] \
                            #       O[j][j+1] * betas_list[j+1] / \
                            #       ()
                            nO += (X[i][j-1] == w) * alphas_list[i][j][a] \
                                  * betas_list[i][j][a] / total1
                ...
                for b in range(self.L):
                    ...
                    for i in range(len(X)):
                        for j in range(1, len(X[i]) + 1):
                            ...
                            #       O[j][j+1] * betas_list[j+1] / \
                            #       ()
                            nA += (X[i][j-1] == w) * alphas_list[i][j][a] \
                                  * betas_list[i][j][a] / total1
                            dA += alphas_list[i][j-1][a] \
                                  * betas_list[i][j-1][a] / total1
                    self.A[a][b] = nA / dA

        pass


    def generate_emission(self, M): # See lecture 14 slide 20
        '''
