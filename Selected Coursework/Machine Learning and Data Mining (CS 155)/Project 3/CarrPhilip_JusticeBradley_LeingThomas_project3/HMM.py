...
class HiddenMarkovModel:
    ...
    def generate_emission(self, M, state0=None):
        '''
    ...
    def generate_reverse_emission(self, M, wordM=None, reverse_return=True):
        '''
        Generates an reverse emission of length M, assuming that the starting
        state is chosen uniformly at random. 

        Arguments:
            M:          Length of the reverse emission to generate.

        Returns:
            emission:   The randomly generated reverse emission as a list.

            states:     The randomly generated reverse states as a list.
        '''
        ...
        if wordM != None:
            rand_var = random.uniform(0, 1)
            curr_state = 0
            # print(len(self.O), len(self.O[0]))
            # print(curr_state, wordM)
            while rand_var > 0 and curr_state < len(self.O):
                rand_var -= self.O[curr_state][wordM]
                curr_state += 1
        ...
        for t in range(M-1):
            ...
            while rand_var > 0 and prev_state < len(self.A):
            ...
            while rand_var > 0 and prev_obs < len(self.O[0]):
                rand_var -= self.O[prev_state][prev_obs]
                prev_obs += 1
