...
vector<float> get_object_bounding_box(Object obj) {
    ...
    float v_y;
    float v_z;
    for (int i = 0; i < (int) obj.vertex_buffer.size(); i++) {
        v_x = obj.vertex_buffer[i].x;
        v_y = obj.vertex_buffer[i].y;
        v_z = obj.vertex_buffer[i].z;
        min_x = min(min_x, v_x);
        max_x = max(max_x, v_x);
        min_y = min(min_y, v_y);
        max_y = max(max_y, v_y);
...
Vec3f calc_vertex_normal(HEV *vertex) {
      ...
      do {
          ...
          if (he->flip) {
              ...
              // Reset the normal vector sum and go in the other direction
              normal.x = 0;
              normal.y = 0;
              normal.z = 0;
...
Mesh_Data* read_object_file(const string filename) {
    ...
    vector<TextureCoords*> *vertex_texture_coords_vector =
        new vector<TextureCoords*>();
    vector<Vec3f*> *vertex_normals_vector = new vector<Vec3f*>();
    ...
    vertex_texture_coords_vector->push_back(NULL);
    vertex_normals_vector->push_back(NULL);
    ...
    while (true) {
        ...
        // Add new vertex uv coordinate pair to texture_coords vector if "vt"
        // found at beginning of line.
        else if (d_type == "vt") {
            TextureCoords *new_vertex_texture_coords = new TextureCoords;
            ifs >> a >> b;
            new_vertex_texture_coords->u = a;
            new_vertex_texture_coords->v = b;
            (*vertex_texture_coords_vector).push_back(
                new_vertex_texture_coords);
        }

        // Add new vertex uv coordinate pair to texture_coords vector if "vt"
        // found at beginning of line.
        else if (d_type == "vn") {
            Vec3f *new_vertex_normal = new Vec3f;
            ifs >> a >> b >> c; // c always = 0 since 2D texture used
            new_vertex_normal->x = a;
            new_vertex_normal->y = b;
            new_vertex_normal->z = c;
            (*vertex_normals_vector).push_back(new_vertex_normal);
        }
        ...
        // Add new face to face vector if "f" found at beginning of line.
            ...
            vector<string> idx1s = split_string(d, '/');
            vector<string> idx2s = split_string(e, '/');
            vector<string> idx3s = split_string(f, '/');

            new_face->vidx1 = stoi(idx1s[0]);
            new_face->vidx2 = stoi(idx2s[0]);
            new_face->vidx3 = stoi(idx3s[0]);

            new_face->vtidx1 = stoi(idx1s[1]);
            new_face->vtidx2 = stoi(idx2s[1]);
            new_face->vtidx3 = stoi(idx3s[1]);

            new_face->vnidx1 = stoi(idx1s[2]);
            new_face->vnidx2 = stoi(idx2s[2]);
            new_face->vnidx3 = stoi(idx3s[2]);
    ...
    mesh_data->vertex_texture_coords = vertex_texture_coords_vector;
    mesh_data->vertex_normals = vertex_normals_vector;
...
/**
 * Average tangent vectors for vertices which have been assigned various
 * different tangent vectors.
 */
void average_tangents(vector<Vertex> &vertex_buffer,
                      vector<Vec3f> &normal_buffer,
                      vector<TextureCoords> &texture_coords_buffer,
                      vector<Vec3f> &tangent_buffer) {
    int buffer_size = (int) vertex_buffer.size();
    for (int i = 0; i < buffer_size; i++) {
        Vertex v1 = vertex_buffer[i];
        Vec3f tangent_sum;
        tangent_sum.x = 0;
        tangent_sum.y = 0;
        tangent_sum.z = 0;

        // Add tangent vectors of equivalent vertices to tangent_sum.
        vector<int> same_vertices;
        for (int j = 0; j < buffer_size; j++) {
            Vertex v2 = vertex_buffer[j];
            if (v1.x == v2.x && v1.y == v2.y && v1.z == v2.z) {
                same_vertices.push_back(j);
                tangent_sum.x += tangent_buffer[j].x;
                tangent_sum.y += tangent_buffer[j].y;
                tangent_sum.z += tangent_buffer[j].z;
            }
        }

        /**
         * Set all applicable vertices' tangent vector to the average tangent
         * of this vertex.
         */
        Vec3f new_tangent;
        new_tangent.x = tangent_sum.x / ((float) same_vertices.size());
        new_tangent.y = tangent_sum.y / ((float) same_vertices.size());
        new_tangent.z = tangent_sum.z / ((float) same_vertices.size());
        for (int j : same_vertices) {
            tangent_buffer[j] = new_tangent;
        }
    }
}
...
void update_object_buffers(Object &obj, bool generate_normals,
    ...
    for (int i = 0; i < (int) (*(mesh_data->faces)).size(); i++) {
        int idx1 = (*(mesh_data->faces))[i]->vidx1;
        int idx2 = (*(mesh_data->faces))[i]->vidx2;
        int idx3 = (*(mesh_data->faces))[i]->vidx3;
        Vec3f normal1, normal2, normal3;

        // Compute vertex normals only if specified by generate_normals.
        if (generate_normals) {
            normal1 = calc_vertex_normal((*hevs)[idx1]);
            normal2 = calc_vertex_normal((*hevs)[idx2]);
            normal3 = calc_vertex_normal((*hevs)[idx3]);
        }
        else {
            normal1 = *(*(mesh_data->vertex_normals))
                          [(*(mesh_data->faces))[i]->vnidx1];
            normal2 = *(*(mesh_data->vertex_normals))
                          [(*(mesh_data->faces))[i]->vnidx2];
            normal3 = *(*(mesh_data->vertex_normals))
                          [(*(mesh_data->faces))[i]->vnidx3];
        }
        ...
        Vertex v1 = *(*(mesh_data->vertices))[idx1];
        Vertex v2 = *(*(mesh_data->vertices))[idx2];
        Vertex v3 = *(*(mesh_data->vertices))[idx3];
        vertex_buffer.push_back(v1);
        vertex_buffer.push_back(v2);
        vertex_buffer.push_back(v3);

        /**
         * Only create the texture coordinates buffer if specified by
         * create_texture_coords_buffer.
         */
        if (create_texture_coords_buffer) {
            int vtidx1 = (*(mesh_data->faces))[i]->vtidx1;
            int vtidx2 = (*(mesh_data->faces))[i]->vtidx2;
            int vtidx3 = (*(mesh_data->faces))[i]->vtidx3;
            TextureCoords uv1 = *(*(mesh_data->vertex_texture_coords))[vtidx1];
            TextureCoords uv2 = *(*(mesh_data->vertex_texture_coords))[vtidx2];
            TextureCoords uv3 = *(*(mesh_data->vertex_texture_coords))[vtidx3];
            texture_coords_buffer.push_back(uv1);
            texture_coords_buffer.push_back(uv2);
            texture_coords_buffer.push_back(uv3);

            /**
             * Compute the vertex tangent vector using the corresponding vertex
             * texture coordinates.
             */
            Vertex deltaPos1;
            deltaPos1.x = v2.x - v1.x;
            deltaPos1.y = v2.y - v1.y;
            deltaPos1.z = v2.z - v1.z;

            Vertex deltaPos2;
            deltaPos2.x = v3.x - v1.x;
            deltaPos2.y = v3.y - v1.y;
            deltaPos2.z = v3.z - v1.z;

            Vec3f deltaUV1;
            deltaUV1.x = uv2.u - uv1.u;
            deltaUV1.y = uv2.v - uv1.v;

            Vec3f deltaUV2;
            deltaUV2.x = uv3.u - uv1.u;
            deltaUV2.y = uv3.v - uv1.v;
    ...
    /**
     * Average the tangent vectors for equivalent vertices for smoother-looking
     * 3D mesh model.
     */
    if (create_texture_coords_buffer) {
        average_tangents(vertex_buffer, normal_buffer, texture_coords_buffer,
                         tangent_buffer);
    }

    // Update the Object data buffers.
    obj.vertex_buffer = vertex_buffer;
    obj.normal_buffer = normal_buffer;
    obj.texture_coords_buffer = texture_coords_buffer;
    obj.tangent_buffer = tangent_buffer;
}
...
Object get_object_from_file(const string filename, bool generate_normals,
    ...
    update_object_buffers(new_object, generate_normals, create_texture_coords_buffer);
        ...
        // phase == 1 when the file has been read up to the "objects:" token.
             ...
             */
                ...
                base_objects.push_back(get_object_from_file(filename,
                    generate_normals, create_texture_coords_buffer));
            }
        }
        else {
            ...
            if (d_type == "texture_image") {
                string texture_image_filename;
                ifs >> texture_image_filename;
                cout << texture_image_filename << endl;
                objects[last_obj_index].texture_image = texture_image_filename;
            }

            else if (d_type == "texture_normals") {
                string texture_normals_filename;
                ifs >> texture_normals_filename;
                objects[last_obj_index].texture_normals =
                    texture_normals_filename;
            }

            else if (d_type == "ambient") {
                float r;
                float g;
                float b;
                ifs >> r >> g >> b;
