                     ...
                     std::vector<HEF*> *hefs)
{
    ...
    for (int i = 0; i < num_faces; ++i)
    {
        ...
        e1->vertex = hevs->at(f->vidx1);
        e2->vertex = hevs->at(f->vidx2);
        e3->vertex = hevs->at(f->vidx3);

        hevs->at(f->vidx1)->out = e1;
        hevs->at(f->vidx2)->out = e2;
        hevs->at(f->vidx3)->out = e3;

        hash_edge(edge_hash, get_edge_key(f->vidx1, f->vidx2), e1);
        hash_edge(edge_hash, get_edge_key(f->vidx2, f->vidx3), e2);
        hash_edge(edge_hash, get_edge_key(f->vidx3, f->vidx1), e3);
